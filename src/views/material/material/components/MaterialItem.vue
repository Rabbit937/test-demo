...
const editAlbumOrFolderFunc = async (type: string) => {
  ...
  if (type === 'confirm') {
    ...
    emit('handleClick', { item: itemRef.value, type: 'materialItem', action: 'edit' })
  } else {
    editAlbumOrFolderState.visble = false
...
const handleItemDelete = (item: any) => {
  const tips = Number(item.type) === 1 ? '专辑' : Number(item.type) === 2 ? '文件夹' : '素材'
  ...
  ElMessageBox.confirm(`您确认删除此${tips}吗?`, '提示', {
    ...
    .then(() => {
      emit('handleClick', { item: item, type: 'materialItem', action: 'delete' })
    })
    .catch(() => {
      ElMessage({
        type: 'info',
    ...
    if (treeDataRef.value) {
      if (Number(treeDataRef.value.AL_ID) === 0) {
        ...
        moveMaterialFunc({
          ...
          relative_dir_id: findParentNodes(props.treeState.treeData, treeDataRef.value.ID).slice(1)
        })

        moveVisible()

        emit('handleClick', { item: moveItemRef.value, type: 'materialItem', action: 'move' })
      }
    } else {
...
const handleEditTagsClose = (tagsParams: any) => {
  ...
  emit('handleClick', { item: itemRef.value, type: 'materialItem', action: 'editTags' })
}
</script>
